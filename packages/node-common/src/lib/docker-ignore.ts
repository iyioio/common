...
export const dockerIgnore = (options?:DockerIgnoreOptions) => new DockerIgnore(options)
...
function make_array<T>(subject:T|T[]):T[] {
...
const KEY_IGNORE = Symbol('dockerignore');
...
// to backslashes.
function cleanPath (file:string) {
...
// https://github.com/golang/go/blob/master/src/path/filepath/path.go
function fromSlash (file:string) {
  if (path.sep === '/') {
    return file
  }
...
export class DockerIgnore {
  ...
  add (pattern:string|string[]|Pattern) {
    this._added = false
    ...
    make_array<Pattern|string>(pattern).forEach(this._addPattern, this)

    return this
  }

  // legacy
  addPattern (pattern:string|string[]|Pattern) {
    return this.add(pattern)
  }

  private _addPattern (pattern:Pattern|string) {
    // https://github.com/kaelzhang/node-ignore/issues/32
    if (pattern && (typeof pattern === 'object') && pattern[KEY_IGNORE]) {
  ...
  private _checkPattern (pattern:any): pattern is string {
  ...
  createFilter () {
    return (path:string) => this._filter(path)
  }
  ...
  private _createRule (pattern:string) {
  ...
  private _match(file:string, rule:DockerIgnoreRule) {
    return this._compile(rule).regexp.test(file)
  }

  // https://github.com/moby/moby/blob/v19.03.8/pkg/fileutils/fileutils.go#L139
  private _compile(rule:DockerIgnoreRule):Required<DockerIgnoreRule> {
    if(rule.regexp) {
      return rule as Required<DockerIgnoreRule>;
    }

    let regStr = '^';

    // Go through the pattern and convert it to a regexp.
    const escapedSlash = path.sep === '\\' ? '\\\\' : path.sep
