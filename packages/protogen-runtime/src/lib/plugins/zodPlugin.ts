    ...
    generate:async ({
        ...
        for(const node of nodes){
            ...
            if(anyAdded){
                ...
                addOutputs(node,path,'',[],outputs,typeOut,typeImports,true);
                addOutputs(node,path,'Scheme',[`import { z } from 'zod';`],outputs,schemeOut,schemeImports,false);
            }
...
const addOutputs=(
    ...
    for(const path in im){
        out.unshift(`import${importAsTypes?' type':''} { ${im[path]?.join(', ')} } from '${path}';`)
    }
...
const addInterface=(
    ...
    const imports=protoGetChildrenByName(node,'$import',false);
    for(const imp of imports){
        if(imp.value?.trim()){
            typeOut.push(`import type ${imp.value?.trim()};`);
        }
    }
    ...
    typeOut.push(`export interface ${node.name}${ext.length?` extends ${ext.join(', ')}`:''}`);
    typeOut.push('{')
    const typeOutStartLength=typeOut.length;
    ...
    if(typeOut.length===typeOutStartLength){
        typeOut.splice(interfaceIndex,0,'// eslint-disable-next-line @typescript-eslint/no-empty-interface');
    }
    typeOut.push('}')
