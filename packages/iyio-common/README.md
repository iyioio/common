...
# HttpClient
The HttpClient class provided a setup of methods for all the common HTTP methods and automatically
...
# Query Objects
Query Objects provide a consist interface for representing custom SQL queries that are constructed
from user input.

For example, a dropdown menu may allow users to view different customer profiles based on events
generated by the customers. As different events are selected in the drop down a query object can
be update to reflect the changes without the need to allow for arbitrary SQL queries created in the
frontend or the need for a dedicated api endpoint.
